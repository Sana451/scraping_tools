...
def read_excel(file_path):
    ...
    df_products = pd.read_excel(file_path, sheet_name=0)
    df_groups = pd.read_excel(file_path, sheet_name=1)
...
def create_bmecat_xml(products_df, groups_df, output_file="PSG-catalog_20.11.2025.xml"):
    ...
    for idx, row in products_df.iterrows():
        ...
        if pd.notna(row['CATALOG_GROUP_ID']):
            article_to_cataloggroup = ET.SubElement(article, "ARTICLE_TO_CATALOGGROUP_MAP")
            ET.SubElement(article_to_cataloggroup, "CATALOG_GROUP_ID").text = str(row['CATALOG_GROUP_ID'])
            ET.SubElement(article_to_cataloggroup, "ARTICLE_TO_CATALOGGROUP_MAP_ORDER").text = str(idx + 1)
...
def main():
    ...
    try:
        products_df, groups_df = read_excel(file_path)
        create_bmecat_xml(products_df, groups_df)
        print(f"BMEcat XML успешно создан: {file_path.replace('.xlsx', '.xml')}")
